...
global $conn;
	...
	$rowAddressArray = array();
	 ...
	 $stmt = $conn->prepare("SELECT user_id, address_array FROM address WHERE user_id=?");
	 ...
	 if( $notExist){
		...
		// add prod id into cartdetails table
	 	 ...
	 	 $stmt2 = $conn->prepare("INSERT INTO address ( user_id, address_array )  VALUES (?,?)");
	   ...
	   /// yes userid exist
		...
		// $information  = $qouteId;
		 ...
		 $tempnewarray = 	 json_encode( $oldarray);
	 	 ...
	 	 $stmt2 = $conn->prepare("UPDATE address SET address_array=? WHERE user_id=?");
